...
# Required: https://github.com/dmlc/mxnet-model-gallery/blob/master/imagenet-21k-inception.md

prefix = "Inseption-21k/Inception"
num_round = 1
model = mx.model.FeedForward.load(prefix, num_round, ctx=mx.cpu(), numpy_batch_size=1)

# load synset (text label)
synset = [l.strip() for l in open('Inseption-21k/synset.txt').readlines()]
...
run_type = 2
if run_type == 1:
    train_index_json = './data/train_index_big.json'
    path = os.path.join('..', 'initial_data', 'train_photos', '*.jpg')
else:
    train_index_json = './data/test_index_big.json'
